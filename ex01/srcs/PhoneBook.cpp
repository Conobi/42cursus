...
/*   Updated: 2022/11/28 03:56:41 by conobi           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
bool PhoneBook::_list_contacts (void) const {
	...
	for (size_t i = 0; i < 9 && this->_contacts[i].get_index() != -1; i++) {
		std::cout
			...
			<< " | " << std::setw(10) << this->_contacts[i].get_first_name().substr(0, 10)
			<< " | " << std::setw(10) << this->_contacts[i].get_last_name().substr(0, 10)
			<< " | " << std::setw(10) << this->_contacts[i].get_nickname().substr(0, 10)
			<< std::endl;
	}
	return (true);
}
