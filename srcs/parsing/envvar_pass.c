...
/*   Updated: 2022/06/09 20:35:53 by conobi           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static char	*str_envvar(t_ctx *c, char *token)
{
	...
	while (
		token[++e.start_kw]
		&& !(token[e.start_kw] == '$') && (ft_isalnum(token[e.start_kw + 1]) || token[e.start_kw + 1] == '_' || c->parser.squoted == 1)
	)
		set_quote_bool(c, token[e.start_kw]);
	e.end_kw = e.start_kw;
	while (token[++e.end_kw]
